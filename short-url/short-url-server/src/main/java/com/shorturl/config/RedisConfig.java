...
public class RedisConfig {
    ...
    public RedisUtils redisUtils(RedisConnectionFactory factory){
        ...
        return new RedisUtils(this.redisTemplate(factory));
    }


    /**
     * 初始化布隆过滤器，放入到spring容器里面
     * Funnel<String> 在这里初始化
     *
     * @return
     */
    @Bean
    public BloomFilterHelper<String> initBloomFilterHelper() {
        return new BloomFilterHelper<>(
                (Funnel<String>) (from, into) -> into.putString(
                        from, Charsets.UTF_8).putString(from,
                        Charsets.UTF_8), 1000000, 0.01);
    }
}